...
void Renderer::init(GPUContext *ctx, SimulationState *simState)
{
    ...
    lines.push_back(Line{.p1 = glm::vec3(0, m_ctx->globals.worldSize.y, 0), .p2 = glm::vec3(m_ctx->globals.worldSize.x, m_ctx->globals.worldSize.y, 0), .thickness = .1f});
    lines.push_back(Line{.p1 = glm::vec3(0, 0, 0), .p2 = glm::vec3(m_ctx->globals.worldSize.x, 0, 0), .thickness = .1f});
    // left & right world bounds
    lines.push_back(Line{.p1 = glm::vec3(0, 0, 0), .p2 = glm::vec3(0, m_ctx->globals.worldSize.y, 0), .thickness = .1f});
    lines.push_back(Line{.p1 = glm::vec3(m_ctx->globals.worldSize.x, 0, 0), .p2 = glm::vec3(m_ctx->globals.worldSize.x, m_ctx->globals.worldSize.y, 0), .thickness = .1f});
